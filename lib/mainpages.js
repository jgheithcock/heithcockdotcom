...
function mapDataToPost(slug, data, content, fields) {
  ...
  fields.forEach((field) => {
    if (field === "slug") {
      ...
      else if (field === "listed") items[field] = false;
      else items[field] = "";
    }
  });
  return items;
...
export function getPosts(slug, fields = ["title", "slug", "date"]) {
  ...
  const atRoot = slug === "";
  const queryFields =
    atRoot && !fields.includes("listed") ? [...fields, "listed"] : fields;
  const posts = contents
    .map((item) => getPostBySlug(item.slug, queryFields))
    .filter((n) => n && (!atRoot || n.listed === true))
    // sort posts by date in order by date (newest first), or ascending by name if no date
    .sort(sortPosts);
  return posts;
}

export function getNavPosts(slug, fields = ["title", "slug", "date"]) {
  /*
    Desired behavior:
      - if there are three children, return next and prev using circular logic
      - if two, then only return next and parent
      - if one (or less), only return parent
  */
  const parentSlug = slug.slice(0, -1).join("/");
  const slugStr = slug.join("/");
  const parentPost = parentSlug
    ? getPostBySlug(parentSlug, fields)
    : { slug: "/", title: "Home" };
  const children = getPosts(parentSlug, fields);
  const findSlug = (s) => s.slug == slugStr;
  const ndex = children.findIndex(findSlug);
  if (ndex === -1 || children.length === 1)
    return { parentPost: parentPost, previousPost: null, nextPost: null };
  if (children.length === 2)
    return {
      parentPost: parentPost,
      previousPost: null,
      nextPost: children[ndex + 1] || children[0] || null,
    };
  return {
    parentPost: parentPost,
    previousPost: children[ndex - 1] || children[children.length - 1] || null,
    nextPost: children[ndex + 1] || children[0] || null,
  };
}
